...
static void tpad_timx_cap_init(uint32_t arr, uint16_t psc)
{
    ...
    gpio_init_struct.Pin  = TPAD_GPIO_PIN;                                          /* 输入捕获的GPIO口 */
    gpio_init_struct.Pull = GPIO_NOPULL;                                            /* 无上下拉电阻 浮空 */
    gpio_init_struct.Mode = GPIO_MODE_AF_PP;                                        /* 输入模式：复用推挽输出 */
    gpio_init_struct.Speed= GPIO_SPEED_FREQ_HIGH;                                   /* 高速 */
    gpio_init_struct.Alternate = TPAD_GPIO_AF;                                      /* PA5 复用为 TIM2_CH1*/
    
    HAL_GPIO_Init(TPAD_GPIO_PORT, &gpio_init_struct);
    
    g_timx_cap_chy_handler.Instance = TPAD_TIMX_CAP;                                /* 定时器 TIM2 */
    g_timx_cap_chy_handler.Init.Prescaler = psc;                                    /* 定时器分频 */
    g_timx_cap_chy_handler.Init.CounterMode = TIM_COUNTERMODE_UP;                   /* 递增计数模式 */
    ...
    g_timx_ic_cap_chy_handler.ICPrescaler = TIM_ICPSC_DIV1;                         /* 不分频 */
    g_timx_ic_cap_chy_handler.ICSelection = TIM_ICSELECTION_DIRECTTI;               /* 映射到TI1上 */
    g_timx_ic_cap_chy_handler.ICFilter = 0;                                         /* 不滤波 */
    HAL_TIM_IC_ConfigChannel(&g_timx_cap_chy_handler, &g_timx_ic_cap_chy_handler, TPAD_TIMX_CAP_CHY); /* 配置TIM2 通道1 */
    
    HAL_TIM_IC_Start(&g_timx_cap_chy_handler,TPAD_TIMX_CAP_CHY);                                                             /* 使能数去捕获和定时器 */
}
