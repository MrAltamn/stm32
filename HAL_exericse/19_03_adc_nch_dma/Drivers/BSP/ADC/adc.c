...
ADC_HandleTypeDef g_adc_nch_handle;         /* ADC句柄 */
...
void adc_nch_dma_init(uint32_t mar)
{
    ...
    __HAL_LINKDMA(&g_adc_nch_handle, DMA_Handle, g_dma_nch_adc_handle);
    
    
    g_adc_nch_handle.Instance = ADC_ADCX;
    g_adc_nch_handle.Init.ClockPrescaler = ADC_CLOCKPRESCALER_PCLK_DIV4;        /* 4分频，ADCCLK = PCLK2/4 = 84/4 = 21Mhz */
    g_adc_nch_handle.Init.Resolution = ADC_RESOLUTION_12B;                      /* 12位模式 */
    g_adc_nch_handle.Init.DataAlign = ADC_DATAALIGN_RIGHT;                      /* 右对齐 */
    g_adc_nch_handle.Init.ScanConvMode = ENABLE;                               /* 非扫描模式 */
    g_adc_nch_handle.Init.ContinuousConvMode = ENABLE;                          /* 连续转换 */
    g_adc_nch_handle.Init.NbrOfConversion = 6;                                  /* 1个转换在规则序列中 也就是只转换规则序列1 */
    g_adc_nch_handle.Init.DiscontinuousConvMode = DISABLE;                      /* 禁止不连续采样模式 */
    g_adc_nch_handle.Init.NbrOfDiscConversion = 0;                              /* 不连续采样通道数为0 */
    g_adc_nch_handle.Init.ExternalTrigConv = ADC_SOFTWARE_START;                /* 软件触发 */
    g_adc_nch_handle.Init.ExternalTrigConvEdge = ADC_EXTERNALTRIGCONVEDGE_NONE; /* 使用软件触发 */
    g_adc_nch_handle.Init.DMAContinuousRequests = ENABLE;                       /* DMA请求 */
    ...
    HAL_ADC_ConfigChannel(&g_adc_nch_handle, &adc_channel);
    ...
    HAL_ADC_Start_DMA(&g_adc_nch_handle, &mar, 0);
}
...
{
//    __HAL_ADC_DISABLE(&g_adc_nch_handle);           /* 先关闭ADC */
//    
...
//    
//    __HAL_ADC_ENABLE(&g_adc_nch_handle);            /* 重新启动ADC */
