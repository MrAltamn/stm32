...
void gtim_timx_cnt_chy_init(uint16_t psc)
{
    ...
    TIM_IC_InitTypeDef timx_cnt_chy_handler;
    GPIO_InitTypeDef gpio_init_struct;
    TIM_SlaveConfigTypeDef timx_slave_handler;
    
    g_timx_cnt_chy_handler.Instance = GTIM_TIMX_CNT;
    g_timx_cnt_chy_handler.Init.Prescaler = psc;
    g_timx_cnt_chy_handler.Init.Period = 65535;
    g_timx_cnt_chy_handler.Init.CounterMode = TIM_COUNTERMODE_UP;
    HAL_TIM_IC_Init(&g_timx_cnt_chy_handler);
    
    timx_cnt_chy_handler.ICPolarity = TIM_ICPOLARITY_RISING;
    timx_cnt_chy_handler.ICSelection = TIM_ICSELECTION_DIRECTTI;
    timx_cnt_chy_handler.ICPrescaler = TIM_ICPSC_DIV1;
    timx_cnt_chy_handler.ICFilter = 0;
    HAL_TIM_IC_ConfigChannel(&g_timx_cnt_chy_handler, &timx_cnt_chy_handler, GTIM_TIMX_CAP_CHY);
    ...
    timx_slave_handler.SlaveMode = TIM_SLAVEMODE_EXTERNAL1;
    timx_slave_handler.InputTrigger = TIM_TS_TI1FP1;
    timx_slave_handler.TriggerPolarity = TIM_TRIGGERPOLARITY_RISING;
    timx_slave_handler.TriggerPrescaler = TIM_TRIGGERPRESCALER_DIV1;
    timx_slave_handler.TriggerFilter = 0x0;
    HAL_TIM_SlaveConfigSynchro(&g_timx_cnt_chy_handler, &timx_slave_handler);
...
{
    __HAL_TIM_DISABLE(&g_timx_cnt_chy_handler);
    g_timxchy_cnt_ofcnt = 0;
    __HAL_TIM_SET_COUNTER(&g_timx_cnt_chy_handler, 0);
    __HAL_TIM_ENABLE(&g_timx_cnt_chy_handler);
}

void GTIM_TIMX_CNT_IRQHandler(void)
{
    if (__HAL_TIM_GET_FLAG(&g_timx_cnt_chy_handler, TIM_FLAG_CC1) != RESET)
    {
        g_timxchy_cnt_ofcnt++;
    }
