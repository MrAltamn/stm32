...
int main(void)
{
    ...
    while (1)
    {
        ...
        if (key == KEY0_PRES)                               /* 高采样率 , 约3Khz波形 */
        {
            ...
            dac_dma_wave_enable(100, 10 - 1, 28 - 1);       /* 300Khz触发频率, 100个点, 可以得到最高3KHz的正弦波. */
        }
        else if (key == KEY1_PRES)                          /* 低采样率 , 约30Khz波形 */
        {
            dac_creat_sin_buf(2048, 10);                    /* 产生正弦波函序列 */
            dac_dma_wave_enable(10, 10 - 1, 28 - 1);        /* 300Khz触发频率, 10个点, 可以得到最高30KHz的正弦波. */
        }
...
//        adcx = adc_get_result();                            /* 得到ADC通道5的转换结果 */
