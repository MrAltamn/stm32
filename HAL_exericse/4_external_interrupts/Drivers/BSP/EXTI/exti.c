...
void exti_init()
{
    ...
    HAL_NVIC_SetPriority(KEY0_INT_IRQn, 0, 2); /* 设置抢占优先级3 子优先级2 */
    ...
    HAL_NVIC_SetPriority(KEY2_INT_IRQn, 2, 2); /* 设置抢占优先级1 子优先级2*/
    HAL_NVIC_EnableIRQ(KEY2_INT_IRQn);         /* 使能中断线2 */
    
    HAL_NVIC_SetPriority(KEY_UP_INT_IRQn, 3, 2); /* 设置抢占优先级0 子优先级2*/
